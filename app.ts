import { AddNumber, CalculateTypes } from "./types/common";

function calculateRectangleArea(rectangle: CalculateTypes) {
  return rectangle.width * rectangle.height;
}

function calculateRectanglePerimeter(rectangle: CalculateTypes) {
  return 2 * (rectangle.width + rectangle.height);
}

function calculateCircleArea(circle: any) {
  return Math.PI * Math.pow(circle.radius, 2);
}

function calculateCirclePerimeter(circle: any) {
